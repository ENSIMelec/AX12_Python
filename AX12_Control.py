...
class AX12_Control:
    ...
    def get_id(self):
        ...
        if dxl_comm_result != COMM_SUCCESS:
            self.logger.error(f"(get_id) {self.packetHandler.getTxRxResult(dxl_comm_result)}")
        elif dxl_error != 0:
            self.logger.error(f"(get_id) {self.packetHandler.getRxPacketError(dxl_error)}")
        else:
            self.logger.info(f"ID actuel du servo: {dxl_id}")
            return dxl_id

    def set_id(self, new_id):
        dxl_comm_result, dxl_error = self.packetHandler.write1ByteTxRx(self.portHandler, self.DXL_ID, ADDR_AX_ID, new_id)
        if dxl_comm_result != COMM_SUCCESS:
            self.logger.error(f"(set_id) {self.packetHandler.getTxRxResult(dxl_comm_result)}")
            return False
        elif dxl_error != 0:
            self.logger.error(f"(set_id) {self.packetHandler.getRxPacketError(dxl_error)}")
            return False
        else:
            self.logger.info(f"ID du servo mis à jour à {new_id}")
            self.DXL_ID = new_id
            return True
    ...
    def get_angle_limit(self):
        ...
        if dxl_comm_result != COMM_SUCCESS:
            self.logger.error(f"(get_cw_angle_limit) {self.packetHandler.getTxRxResult(dxl_comm_result)}")
        elif dxl_error != 0:
            self.logger.error(f"(get_cw_angle_limit) {self.packetHandler.getRxPacketError(dxl_error)}")
        else:
            ...
            if dxl_comm_result != COMM_SUCCESS:
                self.logger.error(f"(get_ccw_angle_limit) {self.packetHandler.getTxRxResult(dxl_comm_result)}")
            elif dxl_error != 0:
                self.logger.error(f"(get_ccw_angle_limit) {self.packetHandler.getRxPacketError(dxl_error)}")
            else:
                if dxl_cw_angle_limit2 > dxl_cw_angle_limit :
                    self.logger.info(f"Limite d'angle: {dxl_cw_angle_limit} à {dxl_cw_angle_limit2}")
                elif dxl_cw_angle_limit > dxl_cw_angle_limit2 :
                    self.logger.info(f"Limite d'angle: {dxl_cw_angle_limit2} à {dxl_cw_angle_limit}")
                else :
                    self.logger.info(f"Limite d'angle: {dxl_cw_angle_limit2} et {dxl_cw_angle_limit} donc continue")
    ...
    def set_angle_limit(self, limit1,limit2):
        ...
        if dxl_comm_result != COMM_SUCCESS:
            self.logger.error(f"(set_cw_angle_limit) {self.packetHandler.getTxRxResult(dxl_comm_result)}")
            return False
        elif dxl_error != 0:
            self.logger.error(f"(set_cw_angle_limit) {self.packetHandler.getRxPacketError(dxl_error)}")
            return False
        else:
            ...
            if dxl_comm_result != COMM_SUCCESS:
                ...
                if limit2 > limit1 :
                    self.logger.info(f"Limite d'angle: {limit1} à {limit2} donc sens horaire (POV LED)")
                elif limit1 > limit2 :
                    self.logger.info(f"Limite d'angle: {limit2} à {limit1} donc sens anti-horaire (POV LED)")
                else :
                    self.logger.info(f"Limite d'angle: {limit2} et {limit1} donc continue")
                return True
    ...
    def get_baudrate(self):
        ...
        if dxl_comm_result != COMM_SUCCESS:
            self.logger.error(f"(get_baudrate) {self.packetHandler.getTxRxResult(dxl_comm_result)}")
        elif dxl_error != 0:
            self.logger.error(f"(get_baudrate) {self.packetHandler.getRxPacketError(dxl_error)}")
        else:
            self.logger.info(f"Baudrate actuel: {dxl_baudrate}")
            return dxl_baudrate
    ...
    def set_baudrate(self, baudrate):
        ...
        if dxl_comm_result != COMM_SUCCESS:
            self.logger.error(f"(set_baudrate) {self.packetHandler.getTxRxResult(dxl_comm_result)}")
            return False
        elif dxl_error != 0:
            self.logger.error(f"(set_baudrate) {self.packetHandler.getRxPacketError(dxl_error)}")
            return False
        else:
            self.logger.info(f"Baudrate mis à jour à {baudrate}")
            self.BAUDRATE = baudrate
            return True
    ...
    def get_present_speed(self):
        ...
        if dxl_comm_result != COMM_SUCCESS:
            self.logger.error(f"(get_present_speed) {self.packetHandler.getTxRxResult(dxl_comm_result)}")
        elif dxl_error != 0:
            self.logger.error(f"(get_present_speed) {self.packetHandler.getRxPacketError(dxl_error)}")
        else:
            self.logger.info(f"Vitesse actuelle: {dxl_speed}")
            return dxl_speed
    ...
    def get_temperature(self):
        ...
        if dxl_comm_result != COMM_SUCCESS:
            self.logger.error(f"(get_temperature) {self.packetHandler.getTxRxResult(dxl_comm_result)}")
        elif dxl_error != 0:
            self.logger.error(f"(get_temperature) {self.packetHandler.getRxPacketError(dxl_error)}")
        else:
            self.logger.info(f"Température actuelle: {dxl_temp}")
            return dxl_temp
    ...
    def get_voltage(self):
        ...
        if dxl_comm_result != COMM_SUCCESS:
            self.logger.error(f"(get_voltage) {self.packetHandler.getTxRxResult(dxl_comm_result)}")
        elif dxl_error != 0:
            self.logger.error(f"(get_voltage) {self.packetHandler.getRxPacketError(dxl_error)}")
        else:
            self.logger.info(f"Tension actuelle: {dxl_voltage / 10.0}V")
    ...
    def get_led(self):
        ...
        if result != COMM_SUCCESS:
            self.logger.error(f"(get_led) {self.packetHandler.getTxRxResult(result)}")
        elif error != 0:
            self.logger.error(f"(get_led) {self.packetHandler.getRxPacketError(error)}")
        else:
            self.logger.info(f"LED status: {led_status}")
            return led_status

    def set_led(self, on_off):
        result, error = self.packetHandler.write1ByteTxRx(self.portHandler, self.DXL_ID, ADDR_AX_LED, on_off)
        if result != COMM_SUCCESS:
            self.logger.error(f"(set_led) {self.packetHandler.getTxRxResult(result)}")
            return False
        elif error != 0:
            self.logger.error(f"(set_led) {self.packetHandler.getRxPacketError(error)}")
            return False
        else:
            self.logger.info(f"LED set to {'on' if on_off else 'off'}")
            return True
    ...
    def get_return_delay_time(self):
        ...
        if result != COMM_SUCCESS:
            self.logger.error(f"(get_return_delay_time) {self.packetHandler.getTxRxResult(result)}")
        elif error != 0:
            self.logger.error(f"(get_return_delay_time) {self.packetHandler.getRxPacketError(error)}")
        else:
            self.logger.info(f"Return delay time: {delay}")
            return delay
    ...
    def set_return_delay_time(self, delay):
        ...
        if result != COMM_SUCCESS:
            self.logger.error(f"(set_return_delay_time) {self.packetHandler.getTxRxResult(result)}")
            return False
        elif error != 0:
            self.logger.error(f"(set_return_delay_time) {self.packetHandler.getRxPacketError(error)}")
            return False
        else:
            self.logger.info(f"Return delay time set to {delay}")
            return True
    ...
    def get_min_voltage_limit(self):
        ...
        if result != COMM_SUCCESS:
            self.logger.error(f"(get_min_voltage_limit) {self.packetHandler.getTxRxResult(result)}")
        elif error != 0:
            self.logger.error(f"(get_min_voltage_limit) {self.packetHandler.getRxPacketError(error)}")
        else:
            self.logger.info(f"Minimum Voltage Limit: {voltage}")
            return voltage
    ...
    def set_min_voltage_limit(self, voltage):
        ...
        if result != COMM_SUCCESS:
            self.logger.error(f"(set_min_voltage_limit) {self.packetHandler.getTxRxResult(result)}")
            return False
        elif error != 0:
            self.logger.error(f"(set_min_voltage_limit) {self.packetHandler.getRxPacketError(error)}")
            return False
        else:
            self.logger.info(f"Minimum Voltage Limit set to {voltage}")
            return True
    ...
    def get_max_voltage_limit(self):
        ...
        if result != COMM_SUCCESS:
            self.logger.error(f"(get_max_voltage_limit) {self.packetHandler.getTxRxResult(result)}")
        elif error != 0:
            self.logger.error(f"(get_max_voltage_limit) {self.packetHandler.getRxPacketError(error)}")
        else:
            self.logger.info(f"Maximum Voltage Limit: {voltage}")
            return voltage
    ...
    def set_max_voltage_limit(self, voltage):
        ...
        if result != COMM_SUCCESS:
            self.logger.error(f"(set_max_voltage_limit) {self.packetHandler.getTxRxResult(result)}")
            return False
        elif error != 0:
            self.logger.error(f"(set_max_voltage_limit) {self.packetHandler.getRxPacketError(error)}")
            return False
        else:
            self.logger.info(f"Maximum Voltage Limit set to {voltage}")
            return True
    ...
    def get_max_torque(self):
        ...
        if result != COMM_SUCCESS:
            self.logger.error(f"(get_max_torque) {self.packetHandler.getTxRxResult(result)}")
        elif error != 0:
            self.logger.error(f"(get_max_torque) {self.packetHandler.getRxPacketError(error)}")
        else:
            self.logger.info(f"Max Torque: {torque}")
            return torque
    ...
    def set_max_torque(self, torque):
        ...
        if result != COMM_SUCCESS:
            self.logger.error(f"(set_max_torque) {self.packetHandler.getTxRxResult(result)}")
            return False
        elif error != 0:
            self.logger.error(f"(set_max_torque) {self.packetHandler.getRxPacketError(error)}")
            return False
        else:
            self.logger.info(f"Max Torque set to {torque}")
            return True
    ...
    def get_torque_enable(self):
        ...
        if result != COMM_SUCCESS:
            self.logger.error(f"(get_torque_enable) {self.packetHandler.getTxRxResult(result)}")
        elif error != 0:
            self.logger.error(f"(get_torque_enable) {self.packetHandler.getRxPacketError(error)}")
        else:
            self.logger.info(f"Torque Enable: {torque_enable}")
    ...
    def set_torque_enable(self, enable):
        ...
        if result != COMM_SUCCESS:
            self.logger.error(f"(set_torque_enable) {self.packetHandler.getTxRxResult(result)}")
            return False
        elif error != 0:
            self.logger.error(f"(set_torque_enable) {self.packetHandler.getRxPacketError(error)}")
            return False
        else:
            self.logger.info(f"Torque Enabled: {enable}")
            return True
    ...
    def get_alarm_led(self):
        ...
        if result != COMM_SUCCESS:
            self.logger.error(f"(get_alarm_led) {self.packetHandler.getTxRxResult(result)}")
        elif error != 0:
            self.logger.error(f"(get_alarm_led) {self.packetHandler.getRxPacketError(error)}")
        else:
            self.logger.info(f"Alarm LED: {alarm_led}")
            return alarm_led
    ...
    def set_alarm_led(self, value):
        ...
        if result != COMM_SUCCESS:
            self.logger.error(f"(set_alarm_led) {self.packetHandler.getTxRxResult(result)}")
            return False
        elif error != 0:
            self.logger.error(f"(set_alarm_led) {self.packetHandler.getRxPacketError(error)}")
            return False
        else:
            self.logger.info(f"Alarm LED set to {value}")
            return True
    ...
    def get_alarm_shutdown(self):
        ...
        if result != COMM_SUCCESS:
            self.logger.error(f"(get_alarm_shutdown) {self.packetHandler.getTxRxResult(result)}")
        elif error != 0:
            self.logger.error(f"(get_alarm_shutdown) {self.packetHandler.getRxPacketError(error)}")
        else:
            self.logger.info(f"Alarm Shutdown: {alarm_shutdown}")
    ...
    def set_alarm_shutdown(self, value):
        ...
        if result != COMM_SUCCESS:
            self.logger.error(f"(set_alarm_shutdown) {self.packetHandler.getTxRxResult(result)}")
            return False
        elif error != 0:
            self.logger.error(f"(set_alarm_shutdown) {self.packetHandler.getRxPacketError(error)}")
            return False
        else:
            self.logger.info(f"Alarm Shutdown set to {value}")
            return True
    ...
    def get_registered_instruction(self):
        ...
        if result != COMM_SUCCESS:
            self.logger.error(f"(get_registered_instruction) {self.packetHandler.getTxRxResult(result)}")
        elif error != 0:
            self.logger.error(f"(get_registered_instruction) {self.packetHandler.getRxPacketError(error)}")
        else:
            self.logger.info(f"Registered Instruction: {instruction}")
            return instruction

    def is_moving(self):
        moving, result, error = self.packetHandler.read1ByteTxRx(self.portHandler, self.DXL_ID, ADDR_AX_MOVING)
        if result != COMM_SUCCESS:
            self.logger.error(f"(is_moving) {self.packetHandler.getTxRxResult(result)}")
        elif error != 0:
            self.logger.error(f"(is_moving) {self.packetHandler.getRxPacketError(error)}")
        else:
            self.logger.info(f"Is Moving: {moving}")
            return moving

    def get_lock(self):
        lock, result, error = self.packetHandler.read1ByteTxRx(self.portHandler, self.DXL_ID, ADDR_AX_LOCK)
        if result != COMM_SUCCESS:
            self.logger.error(f"(get_lock) {self.packetHandler.getTxRxResult(result)}")
        elif error != 0:
            self.logger.error(f"(get_lock) {self.packetHandler.getRxPacketError(error)}")
        else:
            self.logger.info(f"EEPROM Lock: {lock}")
            return lock

    def set_lock(self, lock):
        result, error = self.packetHandler.write1ByteTxRx(self.portHandler, self.DXL_ID, ADDR_AX_LOCK, lock)
        if result != COMM_SUCCESS:
            self.logger.error(f"(set_lock) {self.packetHandler.getTxRxResult(result)}")
            return False
        elif error != 0:
            self.logger.error(f"(set_lock) {self.packetHandler.getRxPacketError(error)}")
            return False
        else:
            self.logger.info(f"EEPROM Lock set to {lock}")
            return True
